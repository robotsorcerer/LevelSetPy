...
class DubinsVehicleRel():
    def __init__(self, grid, u_bound=5, w_bound=5, x=None):
        """
            ...
            Dynamics:
                ...
                \dot{x}_1 = -x_4 + x_5 cos x_3 + w_e x_2
                \dot{x}_2 = x_5 sin x_3 - w_e x_1
                \dot{x}_3 = w_p - w_e
                \dot{x}_4 = a_e
                \dot{x}_5 = a_p
    ...
    def hamiltonian(self, t, data, value_derivs, finite_diff_bundle):
        ...
        """
        p1, p2, p3, p4, p5 = value_derivs[0], value_derivs[1], value_derivs[2], value_derivs[3], value_derivs[4]
        p1_coeff = -self.grid.xs[3] - self.grid.xs[4] * cp.cos(self.grid.xs[2])
        p2_coeff = self.grid.xs[4] * cp.sin(self.grid.xs[2])
    ...
    def dynamics(self):
        ...
        xdot = [
                -x4 + x5 * np.cos(x3) + self.we * x2,
                x5 * np.sin(x3) - self.we * x1,
                self.wp - self.we,
                self.ae,
                self.ap
        ]
