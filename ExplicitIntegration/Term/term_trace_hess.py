...
def termTraceHessian(t, y, schemeData):
    """
     ...
     Computes an approximation to
       ...
       has an icp.t-free stochastic term.  Choose L = R = eye(grid.dim)
     ...
     schemeData.L and schemeData.R may provide matrices in one of three ways
       ...
       3) For general matrices, a function handle to a function with prototype
          ...
          icp.t arguments are the same as those of this function (except that
    ...
    stepBound = 1 / (2 * cp.max(cp.abs(timeStepTrace.flatten(order='F'))))
...
def getMatrix(t, data, schemeData, inM):
    """
     ...
     The icp.t inM may be in one of three formats
     ...
     parameters:
       ...
       inM          Icp.t matrix of type (1), (2) or (3).

       outM         Output matrix of type (2).
    """

    # If the icp.t is a function handle, call the function.
    if(callable(inM)):
    ...
    if isfloat(outM):
        outM = cp.expand_dims(cp.asarray(([[outM]])), 0)
    elif not isinstance(outM, cell):
        error('Icp.t matrix must be a matrix, cell matrix, or function handle.')
