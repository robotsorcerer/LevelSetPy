...
def truncateGrid(gOld, dataOld=None, xmin=None, xmax=None, process=True):
    ...
    gNew = Bundle(dict(dim=gOld.dim, vs=cell(gOld.dim, 1),
                        N = cp.zeros((gOld.dim, 1), dtype=cp.int64),
                        min=cp.zeros((gOld.dim, 1),  dtype=cp.float64),
                        max=cp.zeros((gOld.dim, 1),  dtype=cp.float64),
                        bdry = gOld.bdry
                    ))
    small = 1e-3

    for i in range(gNew.dim):
        idx = cp.logical_and(gOld.vs[i] > xmin[i], gOld.vs[i] < xmax[i])
        gNew.vs[i] = expand(gOld.vs[i][idx], 1)
        gNew.N[i]= len(gNew.vs[i])
        gNew.min[i] = cp.min(gNew.vs[i])

        if gNew.N[i] == 1:
            gNew.max[i] = cp.max(gNew.vs[i]) + small
        else:
            gNew.max[i] = cp.max(gNew.vs[i])
        if gNew.N[i] < gOld.N[i]:
            gNew.bdry[i] = addGhostExtrapolate
    if process:
        gNew = processGrid(gNew)

    if not cp.any(dataOld):
        return gNew
    ...
    if gOld.dim==1:
        if dataOld:
            dataNew = dataOld[cp.logical_and(gOld.vs[0]>xmin, gOld.vs[0]<xmax)]


    elif gOld.dim==2:
        if cp.any(dataOld):
            r = cp.logical_and(gOld.vs[0]>xmin[0], gOld.vs[0]<xmax[0]).squeeze()
            c = cp.logical_and(gOld.vs[1]>xmin[1], gOld.vs[1]<xmax[1]).squeeze()
            dataNew = dataOld[r,:][:,c]

    elif gOld.dim==3:
        if cp.any(dataOld):
            row_idx = cp.logical_and(gOld.vs[0]>xmin[0], gOld.vs[0]<xmax[0]).squeeze()
            col_idx = cp.logical_and(gOld.vs[1]>xmin[1], gOld.vs[1]<xmax[1]).squeeze()
            z_idx   = cp.logical_and(gOld.vs[2]>xmin[2], gOld.vs[2]<xmax[2]).squeeze()
        ...
        if cp.any(dataOld):
            ...
            col_idx = cp.logical_and(gOld.vs[1]>xmin[1], gOld.vs[1]<xmax[1]).squeeze()
            z_idx   = cp.logical_and(gOld.vs[2]>xmin[2], gOld.vs[2]<xmax[2]).squeeze()
            forth_idx = cp.logical_and(gOld.vs[3]>xmin[3], gOld.vs[3]<xmax[3]).squeeze()
