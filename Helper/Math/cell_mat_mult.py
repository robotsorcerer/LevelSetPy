...
def cellMatrixMultiply(A, B):
    if(len(A.shape)>2):
        if(len(B.shape)>2):
            ...
            error('Inp.t B must be a numeric array or a cell matrix')
        ...
        error('Inp.t A must be a numeric array or a cell matrix')


    # If we drop through to here, one of the inp.ts was not a cell matrix.
