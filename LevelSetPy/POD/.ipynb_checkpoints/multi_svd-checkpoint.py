__comment__ = "Ripped off: https://github.com/vortex-exoplanet/VIP/blob/master/vip_hci/pca/svd.py"
...
class SVDecomposer:
    ...
    svd_mode : {'lapack', 'arpack', 'eigen', 'randsvd', 'cupy', 'eigencupy',
        'randcupy', 'pytorch', 'eigenp.torch', 'randpytorch'}, str optional
        ...
        ``eigenp.torch``: offers the same method as with the ``eigen``
    ...
    scaling : {None, "temp-mean", spat-mean", "temp-standard",
        ...
        function. If set to None, the inp.t matrix is left untouched.
        Otherwise:
    ...
    def generate_matrix(self):
        """
        Generate a matrix from the inp.t ``data``. Pixel values in the matrix
...
def svd_wrapper(matrix, mode, ncomp, verbose, full_output=False,
    ...
    -------
    V : numpy ndarray
        The right singular vectors of the inp.t matrix. If ``full_output`` is
        True it returns the left and right singular vectors and the singular
        values of the inp.t matrix. If ``mode`` is set to eigen then only S and
        V are returned.
    ...
    * For ``eigenp.torch`` mode see:
        http://pytorch.org/docs/master/torch.html#torch.eig

    """
    if matrix.ndim != 2:
        raise TypeError('Inp.t matrix is not a 2d array')
    ...
    if full_output:
        ...
        elif mode in ('eigen', 'eigencupy', 'eigenp.torch'):
            return S, V
        else:
            return U, S, V
    else:
...
def randomized_svd_gpu(M, n_components, n_oversamples=10, n_iter='auto',
    ...
    * An implementation of a randomized algorithm for principal component
    ...
    if lib == 'cupy':
        ...
        if transpose:
            # transpose back the results according to the inp.t convention
