# Ripped off Shame McQuarrie's ROM OPInf Code: pre/_reprojection.py
...
def reproject_discrete(f, Vr, x0, niters, U=None):
    ...
    f : callable mapping (n,) ndarray (and (m,) ndarray) to (n,) ndarray
        ...
        a full-order state vector and (optionally) an inp.t vector and returns
    ...
    U : (m,niters-1) or (niters-1) ndarray
        Control inp.ts, one for each iteration beyond the initial condition.
...
def reproject_continuous(f, Vr, X, U=None):
    ...
    f : callable mapping (n,) ndarray (and (m,) ndarray) to (n,) ndarray
        ...
        full-order state vector and (optionally) an inp.t vector and returns
        another full-order state vector.
    ...
    U : (m,k) or (k,) ndarray
        Control inp.ts corresponding to the state trajectories.
