...
def upwindFirstWENO5a(grid, data, dim, generateAll=False):
    ...
    if(generateAll):
        ...
        for i in range(terms):
            indices[i][dim] = cp.arange(i, size(D1, dim) + i - 5, dtype=cp.intp)
        ...
        smoothL = [cp.nan for i in range(len(smooth))]
        ...
        if strcmp(epsilonCalculationMethod, 'constant'):
            ...
            D1squared = D1**2
            epsilonL = 1e-6 * cp.max(D1squared) + 1e-99
            epsilonR = epsilonL
            ...
            for i in range(1, len(indices)):
                epsilon = cp.max(epsilon, D1squared[cp.ix_(*indices[i])])
