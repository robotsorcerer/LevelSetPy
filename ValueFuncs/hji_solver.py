...
def HJIPDE_solve(data0, tau, schemeData, compMethod, extraArgs):
	...
	# Visualization
	if (isfield(extraArgs, 'visualize') and isbundle(extraArgs.visualize, extraArgs))\
		...
		extraArgs.visualize.is_initialized = False
		...
		if isbundle(extraArgs.visualize.initialValueSet, extraArgs.visualize):
			if extraArgs.visualize.is_initialized:
				# plot on ax2
				pass # at a loss here.
			else:
				# put plot on ax1
				...
				if gPlot.dim>3:
					...
					num_plots=3
				lev_interactive = InteractiveVisualizer(params=params, num_plots=num_plots)
